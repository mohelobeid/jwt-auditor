...
def test_extra_findings_raise_score() -> None:
    ...
    two = AuditReport(
        token=token,
        findings=[_finding(Severity.LOW, "a"), _finding(Severity.LOW, "b")],
    )
...
def test_sorted_findings_are_worst_first() -> None:
    ...
    report = AuditReport(
        token=token,
        findings=[_finding(Severity.LOW, "a"), _finding(Severity.CRITICAL, "b")],
    )
...
def test_counts_by_severity() -> None:
    ...
    report = AuditReport(
        token=token,
        findings=[_finding(Severity.HIGH, "a"), _finding(Severity.HIGH, "b")],
    )
